...
CRoutingPPMLanguageModel::CRoutingPPMLanguageModel(CSettingsStore* pSettingsStore, const std::vector<symbol> *pBaseSyms, const std::vector<std::set<symbol> > *pRoutes, bool bRoutesContextSensitive)
:CAbstractPPM(pSettingsStore, static_cast<int>(pRoutes->size())-1, new CRoutingPPMnode(-1), pSettingsStore->GetLongParameter(LP_LM_MAX_ORDER)), NodesAllocated(0), m_NodeAlloc(8192), m_pBaseSyms(pBaseSyms), m_pRoutes(pRoutes), m_bRoutesContextSensitive(bRoutesContextSensitive) {
...
symbol CRoutingPPMLanguageModel::GetBestRoute(Context ctx) {
  ...
  int alpha = m_pSettingsStore->GetLongParameter(LP_LM_ALPHA), beta=m_pSettingsStore->GetLongParameter(LP_LM_BETA);
